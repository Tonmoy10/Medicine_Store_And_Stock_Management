...
namespace MedicineStorePOSAndStockManagement
{
    public partial class FormOwner : Form
    {
        ...
        {
            if (String.IsNullOrEmpty(this.txtCode.Text) || String.IsNullOrEmpty(this.txtMedicineName.Text) ||
        ...
        private void btnConfirmStock_Click(object sender, EventArgs e) //confirm stock button to insert
        {
            try //Handle exception for sql query
            {
                ...
                //insert
                var sql = @"insert into StockInfo values('" + this.txtCode.Text + "','" + this.txtMedicineName.Text + "','" + this.txtMedicinePrice.Text + "','" + this.dtpManufacture.Text + "','" + this.dtpExpire.Text + "','" + this.txtCompany.Text + "','" + this.txtQuantity.Text + "');";
                int count = this.Da.ExecuteDMLQuery(sql);
